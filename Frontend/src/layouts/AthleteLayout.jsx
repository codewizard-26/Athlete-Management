...
function AthleteLayout() {
    ...
    const fetchAthleteData = async () => {
        try {
            ...
            // It's normal if they haven't completed their profile yet
        } finally {
            setLoading(false);
        }
    };
    ...
    const sidebarMenuItems = [
        ...
        { key: "teams", icon: <TeamOutlined />, label: "My Teams" },
        ...
        { key: "performance", icon: <BarChartOutlined />, label: "Performance Stats" },
                        ...
                        {!collapsed && (
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[8px] uppercase tracking-widest text-blue-400 font-bold -mt-0.5">Athlete Hub</span>
                                </div>
                            </div>
                        )}
                        {collapsed && (
                        ...
                        {!collapsed && (
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="min-w-0 flex-grow">
                                    <p className="text-xs font-bold text-white truncate">{user?.name || "Athlete"}</p>
                    ...
                    <header className="w-full flex items-center justify-between py-4 px-6 bg-[#0b0f19]/90 border-b border-white/[0.04] backdrop-blur-sm sticky top-0 z-50">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="hidden md:block">
                                <h2 className="text-xs font-extrabold text-white uppercase tracking-wider">Athlete Portal</h2>
                            </div>
                        </div>

                        {/* Middle/Header display: Logo/Avatar, Athlete Name, Sport, Primary Role */}
                        {loading ? (
                            <Spin size="small" />
                        ) : (
                            user && (
                                <div className="flex items-center space-x-3 bg-white/[0.02] border border-white/[0.05] rounded-xl px-4 py-2 hover:border-white/[0.1] transition-all">
                                    <Avatar size={36} icon={<UserOutlined />} className="bg-blue-600 text-white rounded-lg font-bold shrink-0" />
                                    <div className="min-w-0">
                                        ...
                                        {athleteData?.primaryRole && (
                                            <p className="text-[10px] text-slate-400 mt-1 truncate">
                                                Role: <span className="text-slate-300 font-bold">{athleteData.primaryRole}</span>
                                            </p>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="flex items-center space-x-4">
                            <span className="text-[10px] text-slate-400 hidden lg:inline-flex items-center space-x-1.5 bg-slate-950/40 px-2.5 py-1 rounded-md border border-white/[0.04]">
                                ...
                                <span>Registry ID: <strong className="text-blue-400 font-mono">{user?.email}</strong></span>
                            </span>
                            <Button 
                                type="text" 
                                danger 
                    ...
                    <div className="flex-grow overflow-y-auto bg-[#080b11]">
                        {loading ? (
                            <div className="min-h-[60vh] flex flex-col items-center justify-center">
                                <Spin size="large" />
                                <p className="text-xs text-slate-400 mt-4 tracking-wider">Syncing dashboard...</p>
                            </div>
                        ) : (
                            <Outlet context={{ athleteData, setAthleteData, fetchAthleteData }} />
                        )}
                    </div>
