            ...
            render: (_, record) => (
                <div className="flex items-center space-x-2.5">
                    {(record.logo?.url || (typeof record.logo === "string" && record.logo)) ? (
                        <img src={record.logo?.url || record.logo} alt="Logo" className="w-6 h-6 rounded object-cover border border-border-subtle" />
                    ) : (
                            ...
                            <Card key={t._id} bordered={false} className="border border-border-subtle bg-bg-surface rounded-xl shadow-sm p-4">
                                <div className="flex items-center space-x-3">
                                    {(t.logo?.url || (typeof t.logo === "string" && t.logo)) ? (
                                        <img src={t.logo?.url || t.logo} alt="Logo" className="w-10 h-10 rounded object-cover border border-border-subtle" />
                                    ) : (
